...
```
mf-storefront-demo/
├── package.json                  # workspace root — only dev tooling, no cross-app imports
├── scripts/
│   ├── inspect-all.sh            # monorepo mode: run inspector on all apps + federation
│   ├── inspect-polyrepo.sh       # poly-repo simulation: treat each app as independent
│   └── federation-gate.ts        # programmatic API: score gate for CI
│
│   # Each app below is self-contained — treat as if it were its own repository
│
├── shell/                        # → would be: github.com/acme/shell-mf
│   ├── package.json
│   ├── tsconfig.json
│   ├── webpack.config.js         # ModuleFederationPlugin + MfSharedInspectorPlugin
│   ├── shared-config.json
│   ├── .github/
│   │   └── workflows/
│   │       └── build.yml         # per-app CI — runs inspector, uploads manifest artifact
│   ├── src/
│   │   ├── index.tsx
│   │   ├── App.tsx
│   │   ├── router.tsx
...
│   │       ├── Header.tsx
│   │       └── Layout.tsx
│   └── project-manifest.json     # generated by --write-manifest, uploaded as CI artifact
│
├── catalog/                      # → would be: github.com/acme/catalog-mf
│   ├── package.json
│   ├── tsconfig.json
│   ├── webpack.config.js
│   ├── shared-config.json
...
│
├── checkout/                     # → would be: github.com/acme/checkout-mf
│   ├── package.json
│   ├── tsconfig.json
│   ├── webpack.config.js
│   ├── shared-config.json
